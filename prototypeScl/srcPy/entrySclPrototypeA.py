...
class GlobalCtx(object):
    def __init__(self):
        ...
        self.uniqueIdForEventsTracker = 0 # tracker for unique id's for SclEvent's

    # return unique id for a event
    def retUniqueEventId(self):
        res = self.uniqueIdForEventsTracker
        self.uniqueIdForEventsTracker+=1
        return res
...
class SclStateActionSeqTransitionRuleFunction(object):
    ...
    def calcHashOfForwardCheckDat(self):
        return hash('SclStateActionSeqTransitionRuleFunction') ^ hash('StateActionSeqDat') ^ hash(self.stateActionSeqInDomain.seq[0])
...
class SclStateTerminalCheckerRuleFunction(object):
    ...
    # compute a hash of the whole forward condition
    def calcHashOfForwardCheckDat(self):
        return hash('SclStateTerminalCheckerRuleFunction') ^ hash('StateActionSeqDat') ^ hash(self.stateActionSeqInDomain.seq[0])
...
if __name__ == "__main__":
    ...
    if entryName == 'manualTestLearnPrecondActionConsequenceA':
        ...
        # add rule which decides if the target state has been fullfilled
        if True: # codeblock
            z = StateActionSeqDat(['c'])
            createdRuleFunction = SclStateTerminalCheckerRuleFunction(z,   globalCtx)

            # create the rule
            createdRule = SclRule(
                Type('StateActionSeqDat'),  # input type
                Type('ConseqZ'),  # output type
                createdRuleFunction
            )
        ...
        goalA = TypedInst.makeWithDat(Type('ConseqZ'), None)
